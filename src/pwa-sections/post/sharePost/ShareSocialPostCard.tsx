      ...
      <PostCard>
        ...
        <Stack sx={{ border: `1px solid ${GREY[100]}`, borderRadius: 1, pt: 2, mx: 2 }} spacing={2}>
          ...
          <Stack>
                ...
                onClick={() =>
                  !isShared
                    ? // setSearchParams({ post: post?.sharedSocialPost?.id })
                      push(`${PATH_APP.post.moreMedia}?post=${post?.sharedSocialPost?.id}`)
                    : postShared.editMode
                    ? // setSearchParams({ post: post?.sharedSocialPost?.id })
                      push(`${PATH_APP.post.moreMedia}?post=${post?.sharedSocialPost?.id}`)
                    : // setSearchParams({ post: post?.id })
                      push(`${PATH_APP.post.moreMedia}?post=${post?.id}`)
                }
                container
                spacing={0.5}
                sx={{
            ...
            {media.length >= 2 ? (
              <MediaPostCounter onClick={() => push(`${PATH_APP.post.moreMedia}?post=${post?.sharedSocialPost?.id}`)}>
                <MoreImg>
