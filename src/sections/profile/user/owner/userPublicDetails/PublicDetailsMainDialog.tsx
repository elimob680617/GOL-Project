              ...
              {personSchools?.getPersonSchools?.listDto?.items?.map((school: any) => (
                ...
                >
                  <Stack direction="row" spacing={1} alignItems="center" key={school?.school?.id}>
                    <Icon name="Company-Logo-Empty" color="text.primary" />
                    <Typography variant="body2" color="text.primary" key={school?.school?.id} component="span">
                      <FormattedMessage
                  ...
                  <Box>
                    <IconButton disabled={schoolFetching} onClick={() => handleEditSchool(school)}>
              ...
              {personColleges?.getPersonColleges?.listDto?.items?.map((college) => (
                ...
                >
                  <Stack direction="row" spacing={1} key={college?.collegeDto?.id}>
                    <Icon name="Company-Logo-Empty" color="text.primary" />
                    <Typography variant="body2" color="text.primary" key={college?.collegeDto?.id} component="span">
                      <FormattedMessage
                  ...
                  <IconButton disabled={collegeFetching} onClick={() => handleEditCollege(college as UserCollegeType)}>
                    <Icon name="Edit-Pen" color="text.primary" />
                  </IconButton>
                </Stack>
              ))}
              ...
              {personUniversities?.getPersonColleges?.listDto?.items?.map((university) => (
                ...
                >
                  <Stack direction="row" spacing={1} key={university?.collegeDto?.id}>
                    <Icon name="Company-Logo-Empty" color="text.primary" />
                    <Typography variant="body2" color="text.primary" key={university?.collegeDto?.id} component="span">
                      <FormattedMessage
                        {...NormalPublicDetailsMessages.universityInfo}
                        values={{
                  ...
                  <IconButton disabled={universityFetching} onClick={() => handleEditUniversity(university as any)}>
                    <Icon name="Edit-Pen" color="text.primary" />
                  </IconButton>
                </Stack>
              ))}
              ...
              currentCityData?.getLocation?.listDto?.items?.map((city) => (
                <Box key={city?.city?.id}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Box>
                      <IconButton disabled={isFetching} onClick={() => handleEditCity(city as Location)}>
                        <Icon name="Edit-Pen" color="text.primary" />
                      </IconButton>
                    </Box>
                  </Box>
              ...
              homeTownData?.getLocation?.listDto?.items?.map((city) => (
                <Box key={city?.id}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Box>
                      <IconButton disabled={homeTownFetching} onClick={() => handleEditHomeTown(city as Location)}>
                        <Icon name="Edit-Pen" color="text.primary" />
                      </IconButton>
                    </Box>
                  </Box>
              ...
              relationshipData?.getRelationship?.listDto?.items?.map((rel) => (
                <Box key={rel?.relationshipStatus?.id}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Box>
                      <IconButton
                        disabled={loadingGetRelationship}
                        onClick={() => handleEditRelationship(rel as Relationship)}
                      >
                        <Icon name="Edit-Pen" color="text.primary" />
                      </IconButton>
                    </Box>
            ...
            <Stack spacing={2} sx={{ px: 2, pb: 2 }}>
              ...
              >
                ...
                <Box sx={{ position: 'absolute', left: '50%', transform: 'translate(-50%,-50%)', top: '50%' }}>
                  {/* <Link
                  to={`/profile/user/select-audience-main?audience=${user?.personDto?.joinAudience}`}
      ...
      {openAudience && (
        <Audience
          ...
          onChange={async (val: any) => {
            const response: any = await updateJoinAudience({
              filter: {
                dto: {
                  joinAudience: val as AudienceEnum,
                },
              },
            });
            setRefresh(true);
            if (response?.data?.updateJoinAudience?.isSuccess) {
